...
status=`ps ax | grep "sgagent" | grep -v "grep" |wc -l`
if [ $status -eq 0 ]; then
    err="failed"
    r="${r}{\"filePath\":\"stargate\",\"failReason\":\"process not exist\"},"
    e=1
fi

crontab -l | grep stargate > /dev/null
if [ $? -ne 0 ];then
  err="failed"
  r="${r}{\"filePath\":\"stargate\",\"failReason\":\"crontab not exist\"},"
  e=1
fi
